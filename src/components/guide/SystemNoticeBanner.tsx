    ...
    <View style={styles.wrap}>
      {notices.map((n) => (
        <Pressable
          ...
          accessibilityLabel={`${t('guide.noticePrefix')} ${displayNotificationTitle(n, locale)}`}
        >
          <Ionicons name="megaphone-outline" size={18} color={colors.primary} />
          <View style={styles.body}>
            <Text style={styles.title}>
              {t('guide.noticePrefix')} · {displayNotificationTitle(n, locale)}
            </Text>
            {!!displayNotificationMessage(n, locale) && (
              <Text style={styles.sub}>{displayNotificationMessage(n, locale)}</Text>
            )}
            <Text style={styles.hint}>{t('guide.noticeTapRead')}</Text>
          </View>
        </Pressable>
      ))}
    </View>
